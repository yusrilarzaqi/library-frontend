...
const DataList = () => {
  ...
  const BookCard = ({ book, user }) => (
    <div className={`rounded-lg shadow-sm border p-4 mb-4 ${book.status === 'available' ? 'bg-white' : 'bg-orange-50'}`}>
      ...
      <div className="space-y-2 text-xs text-gray-600 mb-4">
        ...
        {book.status === 'borrowed' && book.borrowedBy && (
          <div className="flex justify-between">
            <span>Dipinjam:</span>
            <span className="text-orange-600">{book.borrowedBy.username}</span>
          </div>
        )}
      </div>
      ...
      {/* Book Detail Modal */}
      {
        showBookModal && selectedBook && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50" onClick={() => setShowBookModal(false)}>
            <div className="bg-white rounded-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto [scrollbar-width:none]" onClick={(e) => e.stopPropagation()}>
              <div className="p-6">
